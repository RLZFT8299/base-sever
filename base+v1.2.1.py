...
class EncoderDecoderApp:
    def __init__(self, root):
        ...
        self.root.geometry("700x650")  # 稍微增加高度以容纳两行按钮
    ...
    def create_widgets(self):
        ...
        button_frame1.grid(row=4, column=1, pady=(5,2), sticky=tk.W)
        ttk.Button(button_frame1, text="执行转换", command=self.convert).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame1, text="清空输入", command=self.clear_input).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame1, text="清空输出", command=self.clear_output).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame1, text="复制结果", command=self.copy_output).pack(side=tk.LEFT, padx=5)

        # 第二行按钮
        button_frame2 = ttk.Frame(main_frame)
        button_frame2.grid(row=5, column=1, pady=(2,5), sticky=tk.W)
        ttk.Button(button_frame2, text="导入文件", command=self.import_file).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame2, text="导出文件", command=self.export_file).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame2, text="交换输入输出", command=self.swap).pack(side=tk.LEFT, padx=5)

        # 输出区域
        ttk.Label(main_frame, text="输出:").grid(row=6, column=0, sticky=tk.NW, pady=5)
        self.output_text = scrolledtext.ScrolledText(main_frame, wrap=tk.WORD, width=70, height=10,
                                                      state=tk.DISABLED)
        self.output_text.grid(row=6, column=1, columnspan=2, sticky=tk.W+tk.E, pady=5)
    ...
    def export_file(self):
        ...
        if not output_content:
            messagebox.showinfo("提示", "输出内容为空，无需导出")
            return
